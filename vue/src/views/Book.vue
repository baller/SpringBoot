  ...
  <div class="home" style ="padding: 10px">
    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      <el-table-column v-if="user.role ==1"
        ...
        <el-table-column prop="isbn" label="图书编号" sortable :width="150" />
    <el-table-column prop="name" label="图书名称" :width="150" />
    <el-table-column prop="author" label="作者" :width="100" />
    <el-table-column prop="genre" label="类型" sortable :width="100" />
    <el-table-column prop="publisher" label="出版社" :width="150" />
    <el-table-column prop="createTime" label="出版时间" sortable :width="150" />
    <el-table-column prop="price" label="价格" sortable :width="50" />
    <el-table-column prop="status" label="状态" sortable :width="100" >
      ...
      <el-table-column fixed="right" label="操作" >
        <template v-slot="scope">
          ...
          <el-button  size="mini" @click ="handleLendbook(scope.row.id,scope.row.isbn,scope.row.name,scope.row.borrownum)" v-if="user.role == 1">管理员借阅</el-button>
          <el-popconfirm title="确认删除?" @confirm="handleDelete(scope.row.id)" v-if="user.role == 1">
            <template #reference>
              <el-button type="danger" size="mini" >删除</el-button>
            </template>
          </el-popconfirm>
          <el-button  size="mini" @click ="handlelend(scope.row.id,scope.row.isbn,scope.row.name,scope.row.borrownum,0)" v-if="user.role === 2 || user.role === 3" :disabled="scope.row.status == 0">借阅</el-button>
          <el-popconfirm title="确认还书?" @confirm="handlereturn(scope.row.id,scope.row.isbn,scope.row.borrownum)" v-if="user.role === 2 || user.role === 3" :disabled="scope.row.status == 1">
            <template #reference>
...
<!--测试,通知对话框-->
    ...
    >
        <el-table :data="outDateBook" style="width: 100%">
          ...
          <el-table-column prop="days_overdue" label="逾期天数" />
          <el-table-column prop="debt" label="罚款" />
        </el-table>
    ...
    <div style="margin: 10px 0">
      ...
      <el-dialog v-model="dialogVisible" title="上架书籍" width="30%">
        <el-form :model="form" label-width="120px">
          ...
          <el-form-item label="类型">
            <el-input style="width: 80%" v-model="form.genre"></el-input>
          </el-form-item>
      ...
      <!-- 图书管理员给读者借阅图书 -->
      <el-dialog v-model="dialogVisible_lendbook" title="图书管理员给读者借阅图书" width="30%">
        <el-form :model="selectedReader" label-width="120px">
            <el-form-item label="选择读者">
              <el-select v-model="selectedReader.id" placeholder="请选择读者" style="width: 80%">
                <el-option v-for="reader in form_lendbook" :key="reader.id" :label="reader.nickName" :value="reader.id"></el-option>
              </el-select>
            </el-form-item>
          </el-form>
        <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogVisible_lendbook = false">取 消</el-button>
        <el-button type="primary" @click="handlelend(this.book_id,this.book_isbn,this.book_name,this.book_bn,1)">确 定</el-button>
      </span>
  ...
  // (this.isbnArray.indexOf(scope.row.isbn)) == -1
    ...
    load(){
      ...
      request.get("user/usersearch",{
        ...
        console.log(res)
        this.form_lendbook = res.data.records
        console.log(this.form_lendbook[2])
      })
    //
        ...
        request.get("/bookwithuser",{
          ...
          for(let i=0; i< this.number; i++){
            ...
            if(dDate < nowDate){
                ...
                let deadDate = new Date(this.bookData[i].deadtime);
                let timeDiff =  nowDate.getTime()-deadDate.getTime();
              ...
              this.outDateBook[this.numOfOutDataBook] = {
                ...
                days_overdue: dayDiff, //逾期天数
                debt:(dayDiff * 0.1).toFixed(1),
              };
    ...
    //读者或者图书管理员借书
    handlelend(id,isbn,name,bn,admin){
    
    this.dialogVisible_lendbook=false

    //学生最多借阅4本书
      if(this.number >=3 && this.user.role == 2){
        ElMessage.warning("您不能再借阅更多的书籍了")
        return;
      }
      //教师最多借阅5本书
      if(this.number >=4 && this.user.role == 3){
        ElMessage.warning("您不能再借阅更多的书籍了")
        return;
      }
      ...
      if(admin==1)
      {
        this.form2.readerId = this.selectedReader.id
      }
      else
      {
      ...
      if(admin==1)
      {
        form3.id = this.selectedReader.id;
      }
      else
      {
        form3.id = this.user.id;
    ...
    handleLendbook(id,isbn,name,bn){
     
      this.dialogVisible_lendbook = true
      this.book_id=id
      this.book_isbn=isbn
      this.book_name=name
      this.book_bn=bn
