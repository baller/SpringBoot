    ...
    <el-table :data="tableData" stripe border="true" @selection-change="handleSelectionChange">
      ...
      <el-table-column fixed="right" label="操作" >
        <template v-slot="scope">
          ...
          <el-popconfirm title="确认缴纳罚款(罚款从校园卡账户扣除)?" @confirm="handlereProlong(scope.row)" v-if="(user.role == 2||user.role == 3) " :disabled="scope.row.days_overdue == 0">
            <template #reference>
              <el-button type="danger" size="mini" :disabled="scope.row.prolong == 0" >缴纳罚款</el-button>
            </template>
          </el-popconfirm>
        </template>
      </el-table-column>
      ...
      if(this.user.role == 1){
        ...
        //检查超时图书数目
            for(let i=0; i< this.tableData.length; i++){
                ...
                let dDate = new Date(this.tableData[i].deadtime);
                var nowDate = new Date();
                if(dDate < nowDate){
                    console.log(dDate)
                    let deadDate = new Date(this.tableData[i].deadtime);
                    let timeDiff =  nowDate.getTime()-deadDate.getTime();
                    let dayDiff = Math.ceil(timeDiff / (1000 * 3600 * 24));
                    this.tableData[i].days_overdue=dayDiff //逾期天数  
                    this.tableData[i].debt=(dayDiff * 0.1).toFixed(1); //罚款
                }
                else{
                    this.tableData[i].days_overdue=0 //逾期天数  
                    this.tableData[i].debt=0 //罚款
                }
          }
        })
      }
