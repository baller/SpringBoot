...
<div>
  ...
  >
    ...
    <el-menu-item index="/librarian" v-if="user.role == 4">  
        <svg class="icon" aria-hidden="true">
          <use xlink:href="#iconreader "></use>
        </svg>
        <span>图书管理员管理</span>
    ...
    <el-menu-item index="/book" v-if="user.role === 2 || user.role === 3">
      <svg class="icon" aria-hidden="true">
        <use xlink:href="#iconbook "></use>
      </svg>
      <span>图书查询</span>
    ...
    <el-menu-item index="/lendrecord" v-if="user.role === 2 || user.role === 3">
      <svg class="icon" aria-hidden="true">
        <use xlink:href="#iconlend-record"></use>
      </svg>
      <span>借阅信息</span>
    </el-menu-item>
    <el-menu-item index="/bookwithuser" v-if="user.role == 2 || user.role == 3">
